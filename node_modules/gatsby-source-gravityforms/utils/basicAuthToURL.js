...
function addBasicAuthToURL(basicAuth, url) {
  ...
  if (url.includes('//')) {
    let protocol = url.split('//')[0]; // Check basic auth

    if (!basicAuth.username || !basicAuth.password) {
      console.log('Basic auth details not included');
    } else {
      console.log('Using basic auth'); // Check protocol exists

      if (protocol.includes('http')) {
        // Re-add // back to string. This is needed later for replacement
        protocol = protocol + '//'; // Add basic auth after protocol

        const authString = protocol + basicAuth.username + ':' + basicAuth.password + '@'; // Replace current protocol with new string
    ...
    console.log('URL structure is different to recommended. This may cause issues.');
